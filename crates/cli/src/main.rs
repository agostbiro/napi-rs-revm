...
fn execute_test_async(test_artifact_path: PathBuf, test_name: String, collect_cache_hit_ratio: bool) -> Result<TestResult> {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()?;

    runtime.block_on(
        runtime.spawn_blocking(move || execute_test(test_artifact_path.as_path(), &test_name, collect_cache_hit_ratio)),
    )?
}
...
fn main() -> Result<()> {
    ...
    let test_result = match args.command {
        Command::ExecuteTestSync => {
            execute_test(args.test_artifact_path.as_path(), &args.test_name, args.cache_hit_ratio)?
        }
        Command::ExecuteTestAsync => execute_test_async(args.test_artifact_path, args.test_name, args.cache_hit_ratio)?,
    };
